#' EPA PBSM snow model
#' @description
#' Process-based snow model with surface sublimation and seven hydroclimate variables (prcp, min/max temperature, wind speed, relative humidity and short/longwave radiations)
#'
#' @details
...
#'
#' @examples
SnowMelt_EPA_Sublim<-function(Date, precip_mm, Tmax_C, Tmin_C, windSp_ms,Relhum_frac,swrad_wm2,lwrad_wm2,
  ...
  # ADDED: Latent Heat of Sublimation (Vaporization + Fusion)
  ...
  Sublimation 		<- rep(0,length(precip_mm))				#  (m)
  ...
  S_incoming 		<- swrad_wm2*86.4		#	Incoming Solar Radiation (kJ/m2/d) (convert W/m2 to kJ/m2/d) (1W/m2=1*3600*24*10^-3)
  S<-S_incoming
  ...
  #--------------------------------------------------------------------------------------
  La 		<- lwrad_wm2*86.4					#	Atmospheric Longwave Radiation (convert W/m2 to kJ/m2/d) (1W/m2=1*3600*24*10^-3)
  ...
  # MODIFIED: Use LatHeatSub instead of lambdaV for snow processes
  E[1] <- LatHeatSub*(rhoa[1]-rhos[1])/rh[1]

  # ADDED: Sublimation Calculation (m)
  # E is in kJ/m2/d. Sublimation mass = E / (LatHeatSub * WaterDens)
  # If E < 0, it is sublimation (loss). If E > 0, it is condensation (gain).
  # We define Sublimation variable as mass LOSS (positive value = loss).
  Sublimation[1] <- -(E[1] / (LatHeatSub * WaterDens))

  # Limit Sublimation: Cannot remove more than available snow
  AvailableMass <- SnowWaterEq[1] + NewSnowWatEq[1]
  if (Sublimation[1] > AvailableMass) {
    Sublimation[1] <- AvailableMass
  }
  # Note: If Sublimation is negative (Condensation), it adds mass.
  ...
  # Update Melt (considering Energy)
  SnowMelt[1] <- max(0,	min((AvailableMass - Sublimation[1]),  # Limit melt to what is left after sublimation
                            (Energy[1]-SnowHeatCap*(AvailableMass - Sublimation[1])*WaterDens*(0-SnowTemp[1]))/(LatHeatFreez*WaterDens) ) )

  # Update Depth and SWE
  # Modified to subtract Sublimation
  SnowDepth[1] <- max(0,(SnowDepth[1] + NewSnowWatEq[1] - SnowMelt[1] - Sublimation[1])*WaterDens/SnowDensity[1])
  SnowWaterEq[1] <- max(0, SnowWaterEq[1] - SnowMelt[1] + NewSnowWatEq[1] - Sublimation[1])
  ...
  for (i in 2:length(precip_m)){
    ...
    # MODIFIED: Use LatHeatSub instead of lambdaV
    E[i] <- LatHeatSub*(rhoa[i]-rhos[i])/rh[i]

    # ADDED: Calculate Sublimation (m)
    # Negative E means sublimation (Loss). Positive E means condensation (Gain).
    Sublimation[i] <- -(E[i] / (LatHeatSub * WaterDens))

    # Available Mass before Sublimation/Melt
    AvailableMass <- SnowWaterEq[i-1] + NewSnowWatEq[i] + R_m_Snow[i]

    # Limit Sublimation to Available Mass
    if (Sublimation[i] > AvailableMass) {
      Sublimation[i] <- AvailableMass
    }
    ...
    # MODIFIED: SnowMelt calculation uses mass remaining AFTER Sublimation
    MassAfterSublimation <- max(0, AvailableMass - Sublimation[i])

    SnowMelt[i] <- max(0,	min( (MassAfterSublimation),  # Cap melt at remaining mass
                               (Energy[i]-SnowHeatCap*(MassAfterSublimation)*WaterDens*(0-SnowTemp[i]))/(LatHeatFreez*WaterDens) )  )

    # MODIFIED: Update SnowDepth and SnowWaterEq with Sublimation term
    # SnowDepth[i] uses mass balance approach
    SnowDepth[i] <- max(0,(AvailableMass - SnowMelt[i] - Sublimation[i])*WaterDens/SnowDensity[i])

    # Update SWE
    SnowWaterEq[i] <- max(0, AvailableMass - SnowMelt[i] - Sublimation[i])	# (m) Equiv depth of water
  }

  # MODIFIED: Added Sublimation_mm to output
  Results<-data.frame(Date, Tmax_C, Tmin_C, precip_mm, R_m*1000, SnowDensity,R_m_Snow*1000,NewSnowWatEq*1000,SnowMelt*1000, Sublimation*1000, NewSnow, SnowDepth, SnowWaterEq*1000)
  colnames(Results)<-c("Date", "MaxT_C", "MinT_C", "Precip_mm", "Rain_mm",'Snow_Density','IceRain_mm',"SnowfallWatEq_mm", "SnowMelt_mm", "Sublimation_mm", "NewSnow_m", "SnowDepth_m", "SnowWaterEq_mm")
  return(Results)
}
