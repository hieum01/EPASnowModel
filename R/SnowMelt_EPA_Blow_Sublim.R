#' EPA PBSM snow model with blowing sublimation and Surface Sublimation
#' @description
#' Process-based snow model with blowing/surface sublimation and seven hydroclimate variables (prcp, min/max temperature, wind speed, relative humidity and short/longwave radiations)
#'
#' @details
#' A modified version of PBSM-S-Curve which simulates snow processes with seven hydroclimate variables. All of hydroclimate variables are from new version of the hybrid climate dataset.
#'
...
#'
#' @examples
SnowMelt_EPA_Blow_Sublim<-function(Date, precip_mm, Tmax_C, Tmin_C, windSp_ms,Relhum_frac,swrad_wm2,lwrad_wm2,
                       lat_deg, slope=0, aspect=0, tempHt=1, windHt=2,groundAlbedo=0.25,SurfEmissiv=0.95,
                       forest=0, startingSnowDepth_m=0,startingSnowDensity_kg_m3=550,Train=10,Tsnow=0,
  ...
  LatHeatSub <- lambdaV + LatHeatFreez # approx 2833.3 kJ/kg

  # ADDED: Constants for Blowing Snow
  z0_snow <- 0.001      # Roughness length of snow (m)
  # Empirical Coefficient for Simplified Blowing Snow Sublimation (Pomeroy & Li approx)
  # Represents the transport efficiency scaling for daily time steps.
  # Note: This is a simplified bulk parameter.
  BlowSub_Coef <- 0.0003
  ...
  rh 	<- log((windHt+0.001)/0.001)*log((tempHt+0.0002)/0.0002)/(0.41*0.41*windSp_ms*86400)	# (day/m) Thermal Resistance
  ...
  # ADDED: Variables for Sublimation
  SurfaceSublimation <- rep(0,length(precip_mm))     # Static surface exchange
  BlowingSublimation <- rep(0,length(precip_mm))     # Blowing snow exchange
  TotalSublimation   <- rep(0,length(precip_mm))     # Total (m)

  Albedo 			<- rep(groundAlbedo,length(precip_mm)) 	#  (-) This will change for days with snow
  Transmis<-transmissivity(Tmax_C,Tmin_C)
  ##	Energy Terms
  H 		<- vector(length=length(precip_mm))	#	Sensible Heat exchanged (kJ/m2/d)
  E 		<- vector(length=length(precip_mm))	#	Vapor Energy	(kJ/m2/d)
  S_incoming 		<- swrad_wm2*86.4		#	Incoming Solar Radiation (kJ/m2/d)
  S<-S_incoming
  S[1]<-(1-Albedo[1])*(1-forest)*S_incoming[1]

  La 		<- lwrad_wm2*86.4					#	Atmospheric Longwave Radiation (kJ/m2/d)
  Lt 		<- vector(length=length(precip_mm))	#	Terrestrial Longwave Radiation (kJ/m2/d)
  G 		<- 173								#	Ground Conduction (kJ/m2/d)
  ...
  # === SURFACE SUBLIMATION ===
  E[1] <- LatHeatSub*(rhoa[1]-rhos[1])/rh[1]
  SurfaceSublimation[1] <- -(E[1] / (LatHeatSub * WaterDens))

  # === BLOWING SNOW SUBLIMATION ===
  # 1. Calculate 10m Wind Speed (Log Profile)
  U10 <- windSp_ms[1] * log(10/z0_snow) / log(windHt/z0_snow)

  # 2. Calculate Threshold Wind Speed (Li & Pomeroy, 1997)
  # U_thresh = 9.43 + 0.18*Tair + 0.0033*Tair^2
  U_thresh <- 9.43 + 0.18*Tmin_C[1] + 0.0033*(Tmin_C[1]^2)

  # 3. Calculate Blowing Sublimation (Simplified Power Law)
  # Only occurs if SnowDepth > 0 and U10 > U_thresh
  if(SnowDepth[1] > 0.005 && U10 > U_thresh) {
    # Simplified Pomeroy approximation adapted for daily step
    BlowingSublimation[1] <- BlowSub_Coef * (U10^2.33) * (1 - Relhum_frac[1])
    # Ensure non-negative
    BlowingSublimation[1] <- max(0, BlowingSublimation[1])
  } else {
    BlowingSublimation[1] <- 0
  }

  # === TOTAL SUBLIMATION & MASS LIMITS ===
  TotalSublimation[1] <- SurfaceSublimation[1] + BlowingSublimation[1]

  AvailableMass <- SnowWaterEq[1] + NewSnowWatEq[1]
  if (TotalSublimation[1] > AvailableMass) {
    TotalSublimation[1] <- AvailableMass
  }
  ...
  # Update Melt and Depth (Using TotalSublimation)
  SnowMelt[1] <- max(0,	min((AvailableMass - TotalSublimation[1]),
                            (Energy[1]-SnowHeatCap*(AvailableMass - TotalSublimation[1])*WaterDens*(0-SnowTemp[1]))/(LatHeatFreez*WaterDens) ) )

  SnowDepth[1] <- max(0,(SnowDepth[1] + NewSnowWatEq[1] - SnowMelt[1] - TotalSublimation[1])*WaterDens/SnowDensity[1])
  SnowWaterEq[1] <- max(0, SnowWaterEq[1] - SnowMelt[1] + NewSnowWatEq[1] - TotalSublimation[1])
  ...
  for (i in 2:length(precip_m)){
    ...
    # === SURFACE SUBLIMATION ===
    E[i] <- LatHeatSub*(rhoa[i]-rhos[i])/rh[i]
    SurfaceSublimation[i] <- -(E[i] / (LatHeatSub * WaterDens))

    # === BLOWING SNOW SUBLIMATION ===
    # 1. Calculate 10m Wind Speed
    U10 <- windSp_ms[i] * log(10/z0_snow) / log(windHt/z0_snow)

    # 2. Calculate Threshold Wind Speed (Li & Pomeroy, 1997)
    U_thresh <- 9.43 + 0.18*Tmin_C[i] + 0.0033*(Tmin_C[i]^2)

    # 3. Calculate Blowing Sublimation
    # Check: Existing Snow, Wind > Threshold, Temp < 0 (no blowing snow if melting/raining usually)
    if(SnowDepth[i-1] > 0.005 && U10 > U_thresh && Tmin_C[i] < 0) {
      BlowingSublimation[i] <- BlowSub_Coef * (U10^2.33) * (1 - Relhum_frac[i])
      BlowingSublimation[i] <- max(0, BlowingSublimation[i])
    } else {
      BlowingSublimation[i] <- 0
    }

    # === TOTAL SUBLIMATION & MASS LIMITS ===
    TotalSublimation[i] <- SurfaceSublimation[i] + BlowingSublimation[i]

    AvailableMass <- SnowWaterEq[i-1] + NewSnowWatEq[i] + R_m_Snow[i]

    if (TotalSublimation[i] > AvailableMass) {
      TotalSublimation[i] <- AvailableMass
    }
    ...
    MassAfterSublimation <- max(0, AvailableMass - TotalSublimation[i])

    SnowMelt[i] <- max(0,	min( (MassAfterSublimation),
                               (Energy[i]-SnowHeatCap*(MassAfterSublimation)*WaterDens*(0-SnowTemp[i]))/(LatHeatFreez*WaterDens) )  )

    SnowDepth[i] <- max(0,(AvailableMass - SnowMelt[i] - TotalSublimation[i])*WaterDens/SnowDensity[i])

    SnowWaterEq[i] <- max(0, AvailableMass - SnowMelt[i] - TotalSublimation[i])
  }

  # MODIFIED: Added BlowingSub_mm and TotalSub_mm to output
  Results<-data.frame(Date, Tmax_C, Tmin_C, precip_mm, R_m*1000, SnowDensity, R_m_Snow*1000, NewSnowWatEq*1000, SnowMelt*1000, SurfaceSublimation*1000, BlowingSublimation*1000, TotalSublimation*1000, NewSnow, SnowDepth, SnowWaterEq*1000)
  colnames(Results)<-c("Date", "MaxT_C", "MinT_C", "Precip_mm", "Rain_mm",'Snow_Density','IceRain_mm',"SnowfallWatEq_mm", "SnowMelt_mm", "SurfSub_mm", "BlowSub_mm", "TotalSub_mm", "NewSnow_m", "SnowDepth_m", "SnowWaterEq_mm")
  return(Results)
}
